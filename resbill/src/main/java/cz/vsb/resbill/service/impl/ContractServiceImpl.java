...
public class ContractServiceImpl implements ContractService {
	...
	@Override
	public ContractHeaderDTO findContractHeaderDTO(Integer contractId) throws ResBillException {
		try {
			return new ContractHeaderDTO(findContract(contractId));
   ...
   */
	@Override
	public List<ContractListDTO> findContractListDTOs(ContractCriteria criteria, Integer offset, Integer limit) throws ResBillException {
		try {
			List<Contract> contracts = findContracts(criteria, offset, limit);
			List<ContractListDTO> dtos = new ArrayList<ContractListDTO>(contracts.size());

			for (Contract contract : contracts) {
				ContractListDTO dto = new ContractListDTO(contract);
				dtos.add(dto);
			}

			return dtos;
		} catch (Exception exc) {
			log.error("An unexpected error occured while finding ContractListDTOs.", exc);
			throw new ResBillException(exc);
		}
	}
	...
	public List<ContractAgendaDTO> findContractAgendaDTOs(ContractCriteria criteria, Integer offset, Integer limit) throws ResBillException {
		try {
			...
			for (Contract contract : contracts) {
				ContractAgendaDTO dto = new ContractAgendaDTO(contract);
				dtos.add(dto);
	...
	public StatisticReportDTO<StatisticContractDTO> findContractStatisticReport(StatisticContractCriteria criteria) throws ResBillException {
		try {
			...
			for (Object[] usage : usages) {
				...
				componentDTO.setCustomerDTO(new CustomerDTO(contract.getCustomer()));
